export async function onRequestGet(context) {
  ...
  try {
    ...
    if (!results || results.length === 0) {
      return new Response(
        `<div id="syllabus-viewer" class="empty-state">
    ...
    const compCards = results
      .map(comp => {
        ...
        return `
        <div class="syllabus-card summary-view"
          ...
          <header style="display: flex; justify-content: space-between; align-items: center;">
            ...
            <span style="font-size: var(--font-size-0); color: var(--text-3); font-weight: bold;">${comp.season_year}</span>
          </header>

          <div style="margin-top: var(--size-2); font-size: var(--font-size-1); color: var(--text-2); display: flex; gap: var(--size-3);">
            <span><strong>Format:</strong> ${displayKind}</span>
            <span><strong>Tracking:</strong> ${comp.sub_kind === 'full' ? 'All Teams' : 'Partial (Us Only)'}</span>
          </div>

          <div style="margin-top: var(--size-2); font-size: var(--font-size-0); color: var(--text-3); text-align: center; border-top: 1px dashed var(--border); padding-top: var(--size-1);">
    ...
    return new Response(`<div id="syllabus-viewer" class="syllabus-list">${compCards}</div>`, {
      headers: { 'Content-Type': 'text/html', 'Cache-Control': 'no-cache' },
    });
  } catch (error) {
    return new Response(`<div id="syllabus-viewer" style="color:var(--red-6)">Error: ${error.message}</div>`, {
      headers: { 'Content-Type': 'text/html' },
    });
  }
}
