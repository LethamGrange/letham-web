...
export async function renderUpdatedResultsList(db) {
  try {
    ...
    if (!results || results.length === 0) {
      return new Response(
        html`<div id="recent-results" class="empty-state">
    ...
    const matchCards = results
      .map(match => {
        return html`
          ...
          >
            <header
              style="display: flex; justify-content: space-between; font-size: var(--font-size-0); color: var(--text-2); margin-bottom: var(--size-2);"
            >
            ...
            <div
              class="match-score-row"
              style="display: flex; justify-content: space-between; align-items: center; font-size: var(--font-size-2);"
            >
              <div class="team-line">
                <span style="${match.final_score_a > match.final_score_b ? 'font-weight: bold;' : ''}"
              ...
              <div class="team-line">
                ...
                <span style="${match.final_score_b > match.final_score_a ? 'font-weight: bold;' : ''}"
                  >${match.team_b_name}</span
                >
              </div>
            </div>
            <div
              style="margin-top: var(--size-2); font-size: var(--font-size-0); color: var(--text-3); text-align: center;"
            >
              Click to view full line score & rosters ▾
            </div>
          </div>
        `;
      })
    ...
    return new Response(html`<div id="recent-results" class="results-list">${matchCards}</div>`, {
      headers: {
    ...
    return new Response(
      html`<div
        id="recent-results"
        style="color: var(--red-6); padding: var(--size-3); border: 1px solid var(--red-3); border-radius: var(--radius-2);"
      >
